...
public sealed class SurveyFormPageTests : IAsyncLifetime
{
    ...
    public async Task The_reflection_is_the_last_panel_and_is_built_from_the_catalog()
    {
        ...
        // Compared against the last BLOCK, not against a C: the panels are ranges of the
        // shuffled sequence now, and none of them is named after a category. The reflection
        // closes the form, after every statement has been asked.
        var lastBlock = html.LastIndexOf("data-tab-label=\"Statements ", StringComparison.Ordinal);
        var reflection = html.IndexOf("data-tab-label=\"End of period\"", StringComparison.Ordinal);

        Assert.True(reflection > lastBlock, "The reflection should come after the statements.");
